...
parser.add_argument('--model', default='say', type=str, choices=['say', 'in', 'wsl', 'rand'], help='which model to use for caching')
...
parser.add_argument('--noise', type=float, default=0.1, help='pixel noise std')
...
def evaluate(data_loader, model, args):
    ...
    with torch.no_grad():
        for i, (images, _) in enumerate(data_loader):
            images = images + args.noise * torch.randn(images.shape)
