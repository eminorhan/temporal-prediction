...
parser.add_argument('--embedding-model', type=str, default='say', choices=['say', 'in', 'wsl', 'rand', 'robust', 'spatial'], help='embedding model')
parser.add_argument('--data', type=str, default='adept', choices=['say', 'intphys', 'adept'], help='data for training the dynamics model')
...
parser.add_argument('--save-interval', type=int, default=1000, help='save model at this rate')
parser.add_argument('--test-intphys', default=False, action='store_true', help='whether to test on intphys dev periodically')
...
if args.data == 'intphys':
    x_W = np.load('../caches_intphys/noise_0.1/intphys_train_' + args.embedding_model + '_256.npz')
    train_data = x_W['x']
elif args.data == 'adept':
    if args.embedding_model == 'spatial':
        projection = np.load('../caches_adept/adept_train_' + args.embedding_model + '_64.npz')['W']
        train_data = np.load('../caches_adept/adept_train_' + args.embedding_model + '_nonflat.npz')['x']
        train_data = np.dot(train_data, projection.T)
        train_data = train_data.reshape(-1, 125, 7*7*64)
    else:
        x_W = np.load('../caches_adept/adept_train_' + args.embedding_model + '_256.npz')
        train_data = x_W['x']
else:
    x_W = np.load('../caches_saycam/' + args.data + '_' + args.embedding_model + '_256adept' + '.npz')  # T x emsize
    train_data = x_W['x']
    trim_end = (train_data.shape[0] // 125) * 125
    train_data = train_data[:trim_end, :]
    train_data = train_data.reshape(-1, 125, 256)

    # concatenate adept training dat
    train_data_2 = np.load('../caches_adept/adept_train_' + args.embedding_model + '_256.npz')['x']
    train_data = np.concatenate((train_data, train_data_2), 0)
...
# Load intphys
###############################################################################
if args.test_intphys:
    def load_intphys(filename):
        intphys = np.load('../caches_intphys/noise_0.1/' + filename)
        intphys_x = intphys['x']
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        if epoch % args.save_interval == 0:
            torch.save({'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict()}, args.embedding_model + '_' + args.data + '_model_' + str(epoch) + '.tar')
