...
parser = argparse.ArgumentParser(description='Evaluate model on ADEPT test set')
parser.add_argument('--workers', default=32, type=int, metavar='N', help='number of data loading workers')
parser.add_argument('--embedding-model', default='in', type=str, choices=['say', 'in', 'rand', 'wsl', 'robust', 'spatial'], help='which model to use for embedding')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.embedding_model == 'say':
    ...
    elif args.embedding_model == 'robust':
        from robustness import model_utils
        from robustness.datasets import ImageNet
        imagenet_ds = ImageNet('/misc/vlgscratch4/LakeGroup/emin/robust_vision/imagenet/')
        arch = models.resnext50_32x4d()
        model_path = 'resnext50_32x4d_l2_eps3.ckpt'
        emb_model, _ = model_utils.make_and_restore_model(arch=arch, dataset=imagenet_ds, resume_path=model_path, parallel=True, add_custom_forward=True)
        emb_model = emb_model.module.model.model
        emb_model = torch.nn.DataParallel(emb_model).cuda()
        emb_model.module.fc = torch.nn.Identity()
    elif args.embedding_model == 'wsl':
        torch.hub.set_dir('/misc/vlgscratch4/LakeGroup/emin/robust_vision/pretrained_models')
        print('=> loading the WSL pre-trained model')
        emb_model = torch.hub.load('facebookresearch/WSL-Images', 'resnext101_32x48d_wsl')        
        ...
        emb_model.fc = torch.nn.Identity()  # dummy layer
        emb_model = torch.nn.DataParallel(emb_model).cuda()
    elif args.embedding_model == 'spatial':
        emb_model = models.resnext50_32x4d(pretrained=True)
        layer_list = list(emb_model.children())[:-2]
        emb_model = torch.nn.Sequential(*layer_list)
        emb_model = torch.nn.DataParallel(emb_model).cuda()
    ...
    if args.embedding_model == 'spatial':    
        ...
        projection = np.load('../caches_adept/adept_train_' + args.embedding_model + '_256.npz')['W']

    projection = torch.from_numpy(projection).float().transpose(0, 1).cuda()

    dyn_model = TransformerModel(3136, 4, 1024, 4, 0.1).cuda()
    checkpoint = torch.load(args.embedding_model + '_adept_model_1000.tar')
    ...
    for d in dir_list:
        ...
        if args.embedding_model == 'spatial':
            ...
            embeddings = embeddings.reshape(-1, 7*7*64)
        else:    
            embeddings = torch.matmul(embeddings, projection)
    ...
    with open(args.embedding_model + '_adept_scores.json', 'w') as json_file:
